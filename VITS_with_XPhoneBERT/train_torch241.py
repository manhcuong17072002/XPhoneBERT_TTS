...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, (x, attention_mask, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(train_loader):
        x, attention_mask, x_lengths = x.cuda(rank, non_blocking=True), attention_mask.cuda(rank,
                                                                                            non_blocking=True), x_lengths.cuda(
...
def evaluate(hps, generator, eval_loader, writer_eval):
    generator.eval()
    with torch.no_grad():
        for batch_idx, (x, attention_mask, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(eval_loader):
